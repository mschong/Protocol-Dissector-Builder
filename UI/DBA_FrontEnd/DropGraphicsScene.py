...
class DropGraphicsScene(QGraphicsScene):
    ...
    def addWidgetToScene(self, widget, pos):
        ...
        if((proxy.scenePos().x() + parent.size().width()) > self.width()):
           self.updateScene(parent.size().width(), True)
        if((proxy.scenePos().y() + parent.size().height()) > self.height()):
            self.updateScene(parent.size().height(), False)
        event.accept()
